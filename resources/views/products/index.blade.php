        ...
        <div class="grid grid-cols-1">
            <div class="bg-white p-6 rounded-lg shadow-md">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('products.create') }}" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded shadow transition duration-200">
                        ADD PRODUCT
                    </a>
                </div>
                            ...
                            @forelse ($products as $product)
                                <tr class="hover:bg-gray-50 border-b border-gray-200">
                                    <td class="px-4 py-3 text-center border border-gray-200">
                                        <img src="{{ asset('/storage/products/'.$product->image) }}" class="rounded w-32 mx-auto">
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-center border border-gray-200">
                                        <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('products.destroy', $product->id) }}" method="POST" class="flex justify-center space-x-2">
                                            <a href="{{ route('products.show', $product->id) }}" class="bg-gray-700 hover:bg-gray-800 text-white text-xs py-1 px-2 rounded shadow">SHOW</a>
                                            <a href="{{ route('products.edit', $product->id) }}" class="bg-blue-500 hover:bg-blue-600 text-white text-xs py-1 px-2 rounded shadow">EDIT</a>
                                            @csrf
                                            @method('DELETE')
