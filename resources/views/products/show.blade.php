    ...
    <div class="container mx-auto mt-10 mb-10 px-4">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white p-6 rounded-lg shadow-md flex justify-center items-center">
                <img src="{{ asset('/storage/products/'.$product->image) }}" class="w-full rounded-md object-cover shadow-sm">
            </div>

            <div class="bg-white p-6 rounded-lg shadow-md md:col-span-2 flex flex-col justify-between">
                <div>
                    <h3 class="text-3xl font-bold text-gray-800 mb-2">{{ $product->title }}</h3>
                    <hr class="mb-4 border-gray-200">
                    
                    <div class="flex space-x-6 mb-6">
                        <div>
                            <span class="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Price</span>
                            <span class="text-xl font-bold text-green-600">{{ "Rp " . number_format($product->price, 2, ',', '.') }}</span>
                        </div>
                        <div class="border-l border-gray-200 pl-6">
                            <span class="block text-xs font-semibold text-gray-400 uppercase tracking-wider">Stock</span>
                            <span class="text-xl font-bold text-gray-700">{{ $product->stock }}</span>
                        </div>
                    </div>
                ...
                <div class="mt-6 border-t border-gray-100 pt-4">
                    <a href="{{ route('products.index') }}" class="inline-block bg-gray-800 hover:bg-gray-900 text-white font-semibold py-2 px-4 rounded shadow transition duration-200 text-sm">
                        BACK TO LIST
                    </a>
                </div>
            </div>
